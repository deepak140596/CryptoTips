package com.localli.deepak.cryptotips.coinlists.favorites;
...
import com.localli.deepak.cryptotips.coinlists.CoinItemAdapter;
...
public class FavoriteListFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    List<CoinItem> coinList, originalCoinList;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        swipeRefreshLayout = view.findViewById(R.id.fragment_favorites_swipe_layout);

        // set recycler view
        recyclerView = view.findViewById(R.id.fragment_favorites_recycler_view);
    ...
    private void prepareFavoritesList(){
        ...
        if(allFavCoinsSet == null || allFavCoinsSet.size() == 0){
            ...
            SharedPrefSimpleDB.insertFavoriteToDB(context,"ethereum");
        }
        for(String s: allFavCoinsSet){
            allFavCoinsString=allFavCoinsString.concat(",").concat(s);
        }

        String GET_COINS_BY_ID_URL = String.format(CoinGeckoService.COINS_BY_ID_URL,
                SharedPrefSimpleDB.getPreferredCurrency(context),allFavCoinsString);
        fetchCoinList(GET_COINS_BY_ID_URL);
